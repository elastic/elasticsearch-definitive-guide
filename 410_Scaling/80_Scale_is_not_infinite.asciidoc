...
The cluster state exists on every node in the cluster, including client nodes.
...
A much better approach is to use <<nested-objects,nested objects>>, with one
