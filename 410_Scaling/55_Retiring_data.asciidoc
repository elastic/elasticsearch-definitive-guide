...
Deleting a whole index is much more efficient than dealing individual
...
by assigning arbitrary tags to each server.  For instance, you culd start a
node with:

    ./bin/elasticsearch --node.box_type strong

The `box_type` parameter is completely aribtrary -- you could have named it
