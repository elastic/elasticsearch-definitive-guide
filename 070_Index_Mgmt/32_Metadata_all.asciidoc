...
indexes the ((("metadata, document", "_all field", secondary-sortas="all field")))((("all field")))values from all other fields as one big string. The `query_string`
...
NOTE: One of the important factors taken into account by the
...
If you decide that you no longer ((("all field", "disabling")))need the `_all` field, you can disable it
with this mapping:
...
by using the `include_in_all` setting, ((("include_in_all setting")))which defaults to `true`. ((("all field", "controlling inclusion in"))) Setting
...
Remember that the `_all` field is just((("all field", "configuring analyzer for")))((("analyzers", "configuring for all field"))) an analyzed `string` field.  It
