...
exists in a separate nested document. ((("nested fields, sorting by")))((("sorting", "by nested fields"))) In order to make the result more
...
ordered by the lowest number of `stars` that each blogpost received. The
...
--------------------------
<1> The `nested` query limits the results to blogposts which received a
    comment in October.
...
<3> The `nested_filter` in the sort clause is the same as the `nested` query in
    the main `query` clause.  See below for the reason.
