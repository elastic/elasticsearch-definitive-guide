...
to examine how to combine multiple queries with the `bool` query. But first, a
quick detour to explain why we <<match-test-data,created our test index>> with
just one primary shard.
