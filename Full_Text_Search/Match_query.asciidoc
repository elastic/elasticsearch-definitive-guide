...
lot of seemingly irrelevant results.  It's a shotgun approach to search.
...
As a percentage, `minimum_should_match` does the right thing. In the example
above with three terms, `75%` would be rounded down to `66.6%` or two out of
the three terms. No matter what you set it to, at least one term must match
...
to examine how to combine queries with the `bool` query.
