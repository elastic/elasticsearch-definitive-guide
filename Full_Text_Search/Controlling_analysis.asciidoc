...
analyzed at index time using the `analyze` API:
...
GET /my_index/_analyze?field=my_type.english <2>
Foxes
--------------------------------------------------
...
<2> Field `english`, which uses the `english` analyzer returns the term
    `fox`.

If we were to run a low-level `term` query for the exact term `fox`, the
`english` field would match but the `title` field would not.
