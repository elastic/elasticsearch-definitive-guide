...
Hunspell dictionaries((("Hunspell stemmer", "obtaining a Hunspell dictionary"))) can be obtained from:
...
The `settings.yml` file contains settings((("Hunspell stemmer", "per-language settings"))) which apply to all of the
...
The quality of dictionaries varies greatly.((("Hunspell stemmer", "strict_affix_parsing"))) Some of the dictionaries that are
...
Lucene to just ignore the broken rules.((("strict_affix_parsing")))
...
token filter((("Hunspell stemmer", "creating a hunspell token filter"))) which uses them:
