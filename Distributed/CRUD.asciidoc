...
In the last chapter, we looked at all the ways to put data into your index
and then retrieve it.  But we glossed over many technical details surrounding
how the data is distributed and fetched from the cluster.  This separation is
done on purpose -- you don't really need to know how data is distributed to
work with Elasticsearch.  It just works.

In this chapter, we are going to dive into those internal, technical details to 
help you how your data is stored in a distributed system.
...
Elasticsearch know which shard a document belongs to?  When we
...
future. In fact, it is a very simple formula:
...
when an index is created and never changed.  If the number of primary shards
ever changed in the future, all previous routing values would be invalid and 
documents would never be found.
...
before being copied to the replicas. These are "write" operations and only the
primary shard is capable of performing "write" operations.
...
shards to be available before even attempting to write data.  This is to prevent
writing data to the "wrong side" of a network partition.  A quorum is defined as:
+
...
+
If a quorum is not available, the primary *does not* write the document and
instead returns an exception to the client.  If the quorum is available, the 
primary will write the document, then push it out to be replicated.  It will
wait for all available shards to respond (either success or failure) before
