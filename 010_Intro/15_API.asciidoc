...
If you are using ((("Java", "clients for Elasticsearch")))Java, then Elasticsearch comes with two built-in clients
...
Both Java clients talk to the cluster over *port 9300*, using((("port 9300 for Java clients"))) the native
...
All other languages can communicate with Elasticsearch((("port 9200 for non-Java clients"))) over *port 9200* using
...
A request to Elasticsearch consists of the same parts as any HTTP request.((("HTTP requests")))((("requests to Elasticsearch"))) For
