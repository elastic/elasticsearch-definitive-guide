...
instance, to count the number of documents in the cluster, we could use this:
...
<4> Any optional query-string parameters (for example `?pretty` will _pretty-print_
...
[source,js]
--------------------------------------------------
curl -XPOST 'localhost:9200/_count?pretty' -d '
{
    "query": {
        "match_all": {}
    }
