...
_aggregations_, which allow you to generate sophisticated analytics over your
...
one in sports.  These aggregations are not precalculated -- they are caculated in the
context of the current query. If we want to know the popular interests of people
called ``Smith'', we can just add the appropriate query into the mix:
