...
[[sense]]
==== Installing Sense
(This step only applies to Kibana before version 5. After that, Sense was renamed Console and
bundled with Kibana.)
...
. Open Sense your web browser by going to `http://localhost:5601/app/sense`. (In Kibana 5+, go to `http://localhost:5601/app/kibana` and click on "Dev Tools".)
