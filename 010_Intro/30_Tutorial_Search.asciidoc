...
This is very easy in Elasticsearch.  We simply execute((("HTTP requests", "retrieving a document with GET"))) an HTTP GET request and
specify the ``address'' of the document -- the index, type and id.((("id", "speccifying in a request")))((("indexes", "specifying index in a request")))((("types", "specifying type in a request")))  Using
...
Query-string search is handy for _ad hoc_ searches((("ad hoc searches"))) from the command line, but
...
The DSL (_Domain Specific Language_) is((("DSL (Domain Specific Language)"))) specified using a JSON request body.
...
try a more advanced, full-text search -- a ((("full-text search")))task which traditional databases
