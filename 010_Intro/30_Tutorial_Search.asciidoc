...
Now that we have some documents stored in Elasticsearch, we can get to work on the
...
specify the ``address'' of the document -- its index, type and id.  Using
...
existing document with an updated version, just `PUT` it again.
...
contains all the matched documents: all of the information that we need to
...
search, since we pass the string we're looking for as a URL query string parameter:
...
We use the same `_search` endpoint in the path as in the previous search, and we add the query itself in
...
The Query DSL (_Domain Specific Language_) is specified using a JSON request body.
We can represent the previous search for all Smith's in the Query DSL like so:
...
parameters, but instead a request body.  This request body contains JSON,
...
When we run this query, the same response is returned as before, but now we get a
