...
purposes of search.((("tokens", "normalizing", "for sorting and collation")))  The final use case that we will consider in this chapter
...
Imagine that we have three `user` documents whose `name` field contain `Boffey`,((("case insensitive sorting")))((("sorting", "case insensitive")))
...
_Collation_ is the process of sorting text into some predefined order.((("collation")))((("Unicode Collation Algorithm (UCA)")))  The
...
The approach described below will probably change in ((("Unicode", "sorting")))((("sorting", "Unicode")))a future version of
Elasticsearch. Check the <<icu-plugin,`icu` plugin>> documentation for the
latest information.

=================================================

The `icu_collation` token filter defaults((("icu_collation token filter"))) to sorting using the DUCET
...
of the token filter ((("German", "collation table for, icu_collation filter using")))using the `language`, `country`, and `variant` parameters
like:
...
The same field can support multiple ((("sorting", "multiple sort orders supported by same field")))sort orders by using a multi-field for
each language:
