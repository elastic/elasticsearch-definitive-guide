...
<1> Find all `location` fields within `1km` of the specified point.
    See http://www.elasticsearch.org/guide/en/elasticsearch/reference/current/common-options.html#distance-units[Distance Units] for
...
the geo-distance calculation only on those points that fall within the bounding
box.
