...
TODO: The only purpose of this sort key is for sorting, not search, so
the sort key is not added to the inverted index but is instead stored on disk
in a data structure known as _doc values_.
...
RAM that is required, especially when dealing with string values.  Of course,
accessing data on disk is somewhat slower than
