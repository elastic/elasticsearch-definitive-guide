...
<<query_string_query,`query_string` and `field` queries>>, by the
`q` query string parameter in the <<search_api,`search` API>> and
by the `percolate` parameter  in the <<index_api,`index`>> and
<<bulk_api,`bulk`>> APIs.
...
As mentioned in <<query_string_query>>, the `default_field` is searched for the
...
The supported regular expression syntax is explained in <<regexp_syntax>>.
...
As explained in <<fuzzy_matching,`fuzzy` matching>>, we can search for terms that are
...
much more reliable to use an explicit <<range_filter,`range` filter>>.
