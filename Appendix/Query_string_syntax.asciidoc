...
<<query-string-query,`query_string` and `field` queries>>, by the
`q` query string parameter in the <<search-api,`search` API>> and
by the `percolate` parameter  in the <<index-api,`index`>> and
<<bulk-api,`bulk`>> APIs.
...
As explained in <<fuzzy-matching,`fuzzy` matching>>, we can search for terms that are
...
much more reliable to use an explicit <<range-filter,`range` filter>>.
...
In contrast, the same query rewritten using the <<match-query,`match` query>>
