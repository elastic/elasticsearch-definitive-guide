...
If you test out a query for ``brown fo'' using ((("partial matching", "index time search-as-you-type", "querying the field")))a simple `match` query:
...
`brown` and `fo`. ((("analyzers", "changing search analyzer from index analyzer"))) We can do this by using the `autocomplete` analyzer at
...
Elasticsearch((("completion suggester"))) takes a completely different approach.  You feed it with a list
of all possible completions and it builds them into a _Finite State
Transducer_, an((("Finite State Transducer"))) optimized data structure which resembles a big graph.  To
