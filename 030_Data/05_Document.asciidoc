...
object, with keys and values.((("objects")))((("JSON", "objects")))((("keys and values"))) A _key_ is the name of a _field_ or _property_,
...
to the top-level or _root object_ which((("root object"))) is serialized into JSON and
stored in Elasticsearch under a unique ID.

=== Document metadata

A document doesn't consist only of its data.((("documents", "metadata"))) It also has
_metadata_ -- information *about* the document.((("metadata, document"))) The three required metadata
elements are:
...
Elasticsearch we use the same _type_ for ((("types")))documents which represent the same
...
underscore, or contain commas.((("types", "names of")))  We shall use `blog` for our type name.
