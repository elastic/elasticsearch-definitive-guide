...
In this example, the `title` and `body` fields are considered to be
_competitive_. We want to find the single *best-matching* field.
...
give preference to fields that contain *both* of the words we are looking for,
rather than the same word repeated in different fields.
...
Disjunction Max Query means: _return documents that match any of these
queries, and calculate the score using the best matching query_:
