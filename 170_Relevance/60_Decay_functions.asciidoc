...
home.((("relevance", "controlling", "using decay functions")))  Maybe they would like to be close to the centre of town, but perhaps
...
operate on numeric fields, date fields or lat-lon geo-points.((("linear function")))((("exp (exponential) function")))((("gauss (Gaussian) function")))  All three take
the same parameters:
...
distance. ((("gauss (Gaussian) function", "in function_score query")))  We could write this query as:
...
individual clauses. ((("weight parameter", "in function_score query"))) The `weight`, which defaults to `1.0`, is multiplied with
