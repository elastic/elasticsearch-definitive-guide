...
operate on numeric fields, date fields or lat-lon geo-points.  Allthree take
the same parameters:
...
image::images/170_10_decay.png["The curves of the decay functions"]
...
* The `gauss` (Gaussian) function is bell-shaped -- it decayd slowly, then
...
£100 should be considered optimal.  If we were to set the `origin` to £100,
then prices below £100 would receive a lower score. Instead, we set both the
`origin` and the `offset` to £50.  That way, the score only decays for any
prices above £100 (`origin + offset`).
