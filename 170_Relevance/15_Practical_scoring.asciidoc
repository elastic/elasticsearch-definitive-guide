...
For multi-term queries, Lucene takes((("Boolean Model"))) the <<boolean-model,Boolean Model>>,
<<tfidf,TF/IDF>>, and the <<vector-space-model,Vector Space Model>> and
combines ((("Term Frequency/Inverse Document Frequency  (TF/IDF) similarity algorithm")))((("Vector Space Model"))) them in a single efficient package which collects matching
...
The query normalization factor (`queryNorm`) is ((("query normalization factor")))((("normalization", "query normalization factor")))an attempt to ``normalize'' a
...
The coordination factor (`coord`) is used to((("coordination factor (coord)")))((("query coordination"))) reward documents which contain a
...
We will talk about _boosting_ a field -- making it ((("boosting", "index time field-level boosting")))more important than other
...
than necessary, this field-level index-time boost is combined with the ((("field length norm")))field
