...
it builds buckets based on a date.  Each bucket is therefore defined as a certain
calendar size (e.g. `1 month` or `2.5 days`, etc).
...
So if your data falls between April and July, you'll only have buckets representing
those months (empty or otherwise).  To get the full year, we need to tell 
Elasticsearch that we want buckets even if they fall _after_ the maximum value.
