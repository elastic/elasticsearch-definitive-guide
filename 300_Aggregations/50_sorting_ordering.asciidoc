...
price was calculated for each term.((("buckets", "multi-value, sorting", "on deeper, nested metrics")))((("metrics", "sorting multi-value buckets by", "deeper, nested metrics")))  It is possible to sort on "deeper" metrics,
...
A `filter` bucket produces((("filter bucket"))) a single bucket:  all documents which match the
