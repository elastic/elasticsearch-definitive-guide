...
favourite songs.((("fielddata", "filtering")))((("aggregations", "fielddata filtering")))  To make it easier for them to manage their music library,
...
a certain size.((("min_segment_size parameter")))  If a segment only holds a few documents, the term frequencies
...
could use a `regex` filter ((("regex filtering")))on tweets to only load hashtags into memory --
