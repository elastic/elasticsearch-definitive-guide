 ...
 *  Doc values are built at index time, not at search time. While fielddata has to
    be built on-the-fly at search time by uninverting the inverted index, doc values
    are pre-built and much faster to initialize.
