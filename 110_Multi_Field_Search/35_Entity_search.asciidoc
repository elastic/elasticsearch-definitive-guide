...
Now we come to a common pattern: cross-field entity search.  With entities
...
using a `dis_max` / `best` query to find the *single* best matching
...
the `multi_match` query instead, and set the `type` to `most` to tell it to
...
==== Problems with the `most` fields approach

The `most` fields approach to entity search has some problems which are not
immediately obvious:
