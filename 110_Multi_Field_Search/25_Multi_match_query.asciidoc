...
The `multi_match` query provides ((("multi-field search", "multi-match query")))((("multi_match queries")))((("match query", "multi_match queries")))us with a convenient shorthand way of running
...
By default it runs as type `best_fields`, which means((("best fields queries", "multi-match queries")))((("dis_max (disjunction max) query", "multi_match query wrapped in"))) that it generates a
