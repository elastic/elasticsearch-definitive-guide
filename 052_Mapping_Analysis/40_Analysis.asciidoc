...
When we query the `_all` field for `2014`, it matches all 12 tweets,
...
When we query the `_all` field for `2014-09-15`, it first analyzes the
query string to produce a query which matches any of the terms `2014`, `09`
...
When we query the `_date` field for `2014-09-15`, it looks for that
