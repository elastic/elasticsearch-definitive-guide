...
almost always better expressed as a ((("non-scoring query", "single-term queries better expressed as")))
non-scoring query, so that they can benefit from <<filter-caching, caching>>:
